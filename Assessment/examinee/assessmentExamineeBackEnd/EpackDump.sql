-- MySQL dump 10.13  Distrib 5.7.32, for Linux (x86_64)
--
-- Host: localhost    Database: oaes_exam_admin_db
-- ------------------------------------------------------
-- Server version	5.7.32-0ubuntu0.18.04.1

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8 */;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;

--
-- Table structure for table `out_epack_header`
--

DROP TABLE IF EXISTS `out_epack_header`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `out_epack_header` (
  `epack_header_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `epack_desc` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `created_on` datetime DEFAULT NULL,
  `created_by` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `epack_status` enum('CREATED','SENT') COLLATE utf8_bin DEFAULT NULL,
  `epack_sent_on` datetime DEFAULT NULL,
  `epack_path` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `center_id` int(10) unsigned NOT NULL,
  PRIMARY KEY (`epack_header_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `out_epack_header`
--

LOCK TABLES `out_epack_header` WRITE;
/*!40000 ALTER TABLE `out_epack_header` DISABLE KEYS */;
INSERT INTO `out_epack_header` VALUES (1,'Nothing','2020-12-01 22:26:16','Author','CREATED',NULL,NULL,1),(2,'Nothing','2020-12-01 22:28:40','Author','CREATED',NULL,NULL,1);
/*!40000 ALTER TABLE `out_epack_header` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `epack1`
--

DROP TABLE IF EXISTS `epack1`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `epack1` (
  `epack1_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `epack_header_id` int(10) unsigned NOT NULL,
  `qp_id` int(10) unsigned NOT NULL,
  `maximum_marks` float NOT NULL,
  `instruction_id` int(10) unsigned DEFAULT NULL,
  `instruction_text` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `duration` int(10) NOT NULL,
  `batch_id` int(10) unsigned NOT NULL,
  `batch_code` varchar(255) COLLATE utf8_bin NOT NULL,
  `batch_start_time` datetime NOT NULL,
  `batch_end_time` datetime NOT NULL,
  `center_id` int(10) unsigned NOT NULL,
  `center_code` varchar(255) COLLATE utf8_bin NOT NULL,
  `center_name` varchar(255) COLLATE utf8_bin NOT NULL,
  `examdrive_id` int(10) unsigned NOT NULL,
  `examdrive_code` varchar(255) COLLATE utf8_bin NOT NULL,
  `examdrive_name` varchar(255) COLLATE utf8_bin NOT NULL,
  `course_master_id` int(10) unsigned NOT NULL,
  `course_code` varchar(255) COLLATE utf8_bin NOT NULL,
  `course_name` varchar(255) COLLATE utf8_bin NOT NULL,
  PRIMARY KEY (`epack1_id`),
  UNIQUE KEY `instruction_id` (`instruction_id`),
  KEY `fk_epack1_epack_header_id` (`epack_header_id`),
  CONSTRAINT `fk_epack1_epack_header_id` FOREIGN KEY (`epack_header_id`) REFERENCES `out_epack_header` (`epack_header_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `epack1`
--

LOCK TABLES `epack1` WRITE;
/*!40000 ALTER TABLE `epack1` DISABLE KEYS */;
INSERT INTO `epack1` VALUES (3,2,4,14,8,'no suplies',30,12,'batch4','2020-12-01 22:23:00','2020-12-02 22:23:00',1,'IIITB','International Institute of Information Technology, Bangalore',5,'test','Test',1,'dbms_101','DBMS'),(4,2,4,14,9,'no extra time',30,12,'batch4','2020-12-01 22:23:00','2020-12-02 22:23:00',1,'IIITB','International Institute of Information Technology, Bangalore',5,'test','Test',1,'dbms_101','DBMS');
/*!40000 ALTER TABLE `epack1` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `epack2`
--

DROP TABLE IF EXISTS `epack2`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `epack2` (
  `epack2_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `epack1_id` int(10) unsigned NOT NULL,
  `qp_id` int(10) unsigned NOT NULL,
  `qp_item_id` int(10) unsigned NOT NULL,
  `item_text` text COLLATE utf8_bin NOT NULL,
  `item_marks` float unsigned NOT NULL,
  `item_type` varchar(255) COLLATE utf8_bin NOT NULL,
  `cognitive_level` enum('REMEMBER','UNDERSTAND','APPLY','ANALYZE','EVALUATE','CREATE') COLLATE utf8_bin DEFAULT NULL,
  PRIMARY KEY (`epack2_id`),
  UNIQUE KEY `qp_item_id` (`qp_item_id`),
  KEY `fk_epack2_epack1_id` (`epack1_id`),
  CONSTRAINT `fk_epack2_epack1_id` FOREIGN KEY (`epack1_id`) REFERENCES `epack1` (`epack1_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `epack2`
--

LOCK TABLES `epack2` WRITE;
/*!40000 ALTER TABLE `epack2` DISABLE KEYS */;
INSERT INTO `epack2` VALUES (10,3,4,10,'<figure class=\"image\"><img src=\"data:image/jpeg;base64,\"></figure> What is a database?',1,'McqSingleCorrect','UNDERSTAND'),(11,3,4,11,'What is a  PK?',1,'McqSingleCorrect','UNDERSTAND'),(12,3,4,12,'What is a FK?',1,'McqMultiCorrect','APPLY'),(13,3,4,13,'UK and PK are same.',1,'True/False','APPLY'),(14,3,4,14,'What do you mean by one to many relationship between Teacher and Class table?',2,'McqSingleCorrect','EVALUATE'),(15,3,4,15,'In one-to-many relationship the table on \'many\' side is called',2,'McqSingleCorrect','ANALYZE'),(16,3,4,16,'In which state one gathers and list all the necessary fields for the database design project.',2,'McqSingleCorrect','UNDERSTAND'),(17,3,4,17,' 	\nA database has a built-in capability to create, process and administer itself.',2,'True/False','CREATE'),(18,3,4,18,'Enterprise Resource Planning (ERP) is an example of a single user database.',2,'True/False','APPLY');
/*!40000 ALTER TABLE `epack2` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `epack3`
--

DROP TABLE IF EXISTS `epack3`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `epack3` (
  `epack3_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `epack2_id` int(10) unsigned NOT NULL,
  `qp_item_id` int(10) unsigned NOT NULL,
  `item_option_id` int(10) unsigned NOT NULL,
  `option_text` varchar(255) COLLATE utf8_bin NOT NULL,
  PRIMARY KEY (`epack3_id`),
  KEY `fk_epack3_epack2_id` (`epack2_id`),
  CONSTRAINT `fk_epack3_epack2_id` FOREIGN KEY (`epack2_id`) REFERENCES `epack2` (`epack2_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=57 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `epack3`
--

LOCK TABLES `epack3` WRITE;
/*!40000 ALTER TABLE `epack3` DISABLE KEYS */;
INSERT INTO `epack3` VALUES (29,16,16,21,'Data Definition'),(30,16,16,22,'Data Refinement'),(31,16,16,23,'Establishing Relationship'),(32,16,16,24,'None Of The Above'),(33,17,17,1,'True'),(34,17,17,1,'False'),(35,18,18,2,'True'),(36,18,18,2,'False'),(37,10,10,1,'Collection of related data'),(38,10,10,2,'Collection of data'),(39,10,10,3,'Collection of words'),(40,10,10,4,'Collection of people'),(41,11,11,5,'Primary Key'),(42,11,11,6,'Personal Key'),(43,11,11,7,'Person Key'),(44,11,11,8,'Primary Keyword'),(45,12,12,9,'Foreign Key'),(46,12,12,10,'Foreign Keyword'),(47,12,12,11,'Fashion Key'),(48,12,12,12,'Forward Key'),(49,14,14,13,'One class may have many teachers'),(50,14,14,14,'One teacher can have many classes'),(51,14,14,15,'Many classes may have many teachers'),(52,14,14,16,'Many teachers may have many classes'),(53,15,15,17,'Parent'),(54,15,15,18,'Child'),(55,15,15,19,'Sister'),(56,15,15,20,'Master');
/*!40000 ALTER TABLE `epack3` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `epack4`
--

DROP TABLE IF EXISTS `epack4`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `epack4` (
  `epack4_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `epack1_id` int(10) unsigned NOT NULL,
  `batch_id` int(10) unsigned NOT NULL,
  `examinee_batch_id` int(10) unsigned NOT NULL,
  `examinee_id` int(10) unsigned NOT NULL,
  `examinee_code` varchar(255) COLLATE utf8_bin NOT NULL,
  `examinee_name` varchar(255) COLLATE utf8_bin NOT NULL,
  `examinee_password` varchar(255) COLLATE utf8_bin NOT NULL,
  `examinee_branch` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `examinee_email` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `examinee_college` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  PRIMARY KEY (`epack4_id`),
  KEY `fk_epack4_epack1_id` (`epack1_id`),
  CONSTRAINT `fk_epack4_epack1_id` FOREIGN KEY (`epack1_id`) REFERENCES `epack1` (`epack1_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `epack4`
--

LOCK TABLES `epack4` WRITE;
/*!40000 ALTER TABLE `epack4` DISABLE KEYS */;
INSERT INTO `epack4` VALUES (4,3,12,14,1,'S1','ABC','abc','CSE','ABC@iiitb.org','IIITB'),(5,3,12,15,3,'S3','Z','z','ECE','null','null'),(6,3,12,16,5,'S5','X','x','null','null','null');
/*!40000 ALTER TABLE `epack4` ENABLE KEYS */;
UNLOCK TABLES;
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;

/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;

-- Dump completed on 2020-12-01 22:28:42
-- Dump execute Success
2020-12-01 22:32:16.296  WARN 1475 --- [nio-8080-exec-5] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [batch] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:32:16.298  WARN 1475 --- [nio-8080-exec-5] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [examinee] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:32:51.419  WARN 1475 --- [nio-8080-exec-1] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [batch] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:32:51.420  WARN 1475 --- [nio-8080-exec-1] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [examinee] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:37:16.655  WARN 1475 --- [nio-8080-exec-9] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [batch] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:37:16.655  WARN 1475 --- [nio-8080-exec-9] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [examinee] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:37:28.579  WARN 1475 --- [nio-8080-exec-1] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [batch] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
2020-12-01 22:37:28.580  WARN 1475 --- [nio-8080-exec-1] o.h.p.entity.AbstractEntityPersister     : HHH000502: The [examinee] property of the [com.iiitb.assessmentBackEnd.examineeBatch.AsExamineeBatch] entity was modified, but it won't be updated because the property is immutable.
creating dump
----SQL backup file generated: mydb_abackup.sql----
2020-12-01 23:11:36.251  INFO 1475 --- [extShutdownHook] j.LocalContainerEntityManagerFactoryBean : Closing JPA EntityManagerFactory for persistence unit 'default'
2020-12-01 23:11:36.261  INFO 1475 --- [extShutdownHook] o.s.s.concurrent.ThreadPoolTaskExecutor  : Shutting down ExecutorService 'applicationTaskExecutor'
2020-12-01 23:11:36.263  INFO 1475 --- [extShutdownHook] com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Shutdown initiated...
2020-12-01 23:11:36.291  INFO 1475 --- [extShutdownHook] com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Shutdown completed.
