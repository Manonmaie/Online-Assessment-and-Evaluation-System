-- MySQL dump 10.13  Distrib 8.0.12, for osx10.13 (x86_64)
--
-- Host: localhost    Database: oaes_assessment_db
-- ------------------------------------------------------
-- Server version	8.0.12

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
 SET NAMES utf8mb4 ;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;

--
-- Table structure for table `out_rpack_header`
--

DROP TABLE IF EXISTS `out_rpack_header`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `out_rpack_header` (
  `rpack_header_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rpack_desc` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `created_on` datetime DEFAULT NULL,
  `created_by` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  `rpack_status` enum('CREATED','SENT') COLLATE utf8_bin DEFAULT NULL,
  `rpack_sent_on` datetime DEFAULT NULL,
  `rpack_path` varchar(255) COLLATE utf8_bin DEFAULT NULL,
  PRIMARY KEY (`rpack_header_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `out_rpack_header`
--

LOCK TABLES `out_rpack_header` WRITE;
/*!40000 ALTER TABLE `out_rpack_header` DISABLE KEYS */;
INSERT INTO `out_rpack_header` VALUES (1,'Nothing','2020-12-01 22:39:14','Author','SENT','2020-12-01 22:39:14',NULL),(2,'Nothing','2020-12-01 23:12:11','Author','SENT','2020-12-01 23:12:11',NULL);
/*!40000 ALTER TABLE `out_rpack_header` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `rpack1`
--

DROP TABLE IF EXISTS `rpack1`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `rpack1` (
  `rpack1_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rpack_header_id` int(10) unsigned NOT NULL,
  `qp_id` int(10) unsigned NOT NULL,
  `maximum_marks` float NOT NULL,
  `duration` int(10) NOT NULL,
  `batch_id` int(10) unsigned NOT NULL,
  PRIMARY KEY (`rpack1_id`),
  KEY `fk_rpack1_rpack_header_id` (`rpack_header_id`),
  CONSTRAINT `fk_rpack1_rpack_header_id` FOREIGN KEY (`rpack_header_id`) REFERENCES `out_rpack_header` (`rpack_header_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `rpack1`
--

LOCK TABLES `rpack1` WRITE;
/*!40000 ALTER TABLE `rpack1` DISABLE KEYS */;
INSERT INTO `rpack1` VALUES (2,2,4,14,30,12);
/*!40000 ALTER TABLE `rpack1` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `rpack2`
--

DROP TABLE IF EXISTS `rpack2`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `rpack2` (
  `rpack2_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rpack1_id` int(10) unsigned NOT NULL,
  `qp_id` int(10) unsigned NOT NULL,
  `qp_item_id` int(10) unsigned NOT NULL,
  `item_text` text COLLATE utf8_bin NOT NULL,
  `item_marks` float unsigned NOT NULL,
  `item_type` varchar(255) COLLATE utf8_bin NOT NULL,
  `cognitive_level` enum('REMEMBER','UNDERSTAND','APPLY','ANALYZE','EVALUATE','CREATE') COLLATE utf8_bin DEFAULT NULL,
  PRIMARY KEY (`rpack2_id`),
  KEY `fk_rpack2_rpack1_id` (`rpack1_id`),
  CONSTRAINT `fk_rpack2_rpack1_id` FOREIGN KEY (`rpack1_id`) REFERENCES `rpack1` (`rpack1_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `rpack2`
--

LOCK TABLES `rpack2` WRITE;
/*!40000 ALTER TABLE `rpack2` DISABLE KEYS */;
INSERT INTO `rpack2` VALUES (10,2,4,10,'<figure class=\"image\"><img src=\"data:image/jpeg;base64,\"></figure> What is a database?',1,'McqSingleCorrect','UNDERSTAND'),(11,2,4,11,'What is a  PK?',1,'McqSingleCorrect','UNDERSTAND'),(12,2,4,12,'What is a FK?',1,'McqMultiCorrect','APPLY'),(13,2,4,13,'UK and PK are same.',1,'True/False','APPLY'),(14,2,4,14,'What do you mean by one to many relationship between Teacher and Class table?',2,'McqSingleCorrect','EVALUATE'),(15,2,4,15,'In one-to-many relationship the table on \'many\' side is called',2,'McqSingleCorrect','ANALYZE'),(16,2,4,16,'In which state one gathers and list all the necessary fields for the database design project.',2,'McqSingleCorrect','UNDERSTAND'),(17,2,4,17,' 	\nA database has a built-in capability to create, process and administer itself.',2,'True/False','CREATE'),(18,2,4,18,'Enterprise Resource Planning (ERP) is an example of a single user database.',2,'True/False','APPLY');
/*!40000 ALTER TABLE `rpack2` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `rpack3`
--

DROP TABLE IF EXISTS `rpack3`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `rpack3` (
  `rpack3_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rpack2_id` int(10) unsigned NOT NULL,
  `qp_item_id` int(10) unsigned NOT NULL,
  `item_option_id` int(10) unsigned NOT NULL,
  `option_text` varchar(255) COLLATE utf8_bin NOT NULL,
  PRIMARY KEY (`rpack3_id`),
  KEY `fk_rpack3_rpack2_id` (`rpack2_id`),
  CONSTRAINT `fk_rpack3_rpack2_id` FOREIGN KEY (`rpack2_id`) REFERENCES `rpack2` (`rpack2_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=57 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `rpack3`
--

LOCK TABLES `rpack3` WRITE;
/*!40000 ALTER TABLE `rpack3` DISABLE KEYS */;
INSERT INTO `rpack3` VALUES (29,16,16,21,'Data Definition'),(30,16,16,22,'Data Refinement'),(31,16,16,23,'Establishing Relationship'),(32,16,16,24,'None Of The Above'),(33,17,17,1,'True'),(34,17,17,1,'False'),(35,18,18,2,'True'),(36,18,18,2,'False'),(37,10,10,1,'Collection of related data'),(38,10,10,2,'Collection of data'),(39,10,10,3,'Collection of words'),(40,10,10,4,'Collection of people'),(41,11,11,5,'Primary Key'),(42,11,11,6,'Personal Key'),(43,11,11,7,'Person Key'),(44,11,11,8,'Primary Keyword'),(45,12,12,9,'Foreign Key'),(46,12,12,10,'Foreign Keyword'),(47,12,12,11,'Fashion Key'),(48,12,12,12,'Forward Key'),(49,14,14,13,'One class may have many teachers'),(50,14,14,14,'One teacher can have many classes'),(51,14,14,15,'Many classes may have many teachers'),(52,14,14,16,'Many teachers may have many classes'),(53,15,15,17,'Parent'),(54,15,15,18,'Child'),(55,15,15,19,'Sister'),(56,15,15,20,'Master');
/*!40000 ALTER TABLE `rpack3` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `rpack4`
--

DROP TABLE IF EXISTS `rpack4`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
 SET character_set_client = utf8mb4 ;
CREATE TABLE `rpack4` (
  `rpack4_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rpack2_id` int(10) unsigned NOT NULL,
  `qp_id` int(10) unsigned NOT NULL,
  `batch_id` int(10) unsigned NOT NULL,
  `examinee_batch_id` int(10) unsigned NOT NULL,
  `examinee_id` int(10) unsigned NOT NULL,
  `qp_item_id` int(10) unsigned NOT NULL,
  `response_id` int(10) unsigned NOT NULL,
  `response_text` varchar(511) COLLATE utf8_bin NOT NULL,
  PRIMARY KEY (`rpack4_id`),
  KEY `fk_rpack4_rpack2_id` (`rpack2_id`),
  CONSTRAINT `fk_rpack4_rpack2_id` FOREIGN KEY (`rpack2_id`) REFERENCES `rpack2` (`rpack2_id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=49 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `rpack4`
--

LOCK TABLES `rpack4` WRITE;
/*!40000 ALTER TABLE `rpack4` DISABLE KEYS */;
INSERT INTO `rpack4` VALUES (37,16,4,12,15,3,16,7,'Data Definition'),(38,17,4,12,15,3,17,8,'True'),(39,18,4,12,15,3,18,10,'True'),(40,10,4,12,14,1,10,1,'Collection of related data'),(41,10,4,12,15,3,10,9,'Collection of related data'),(42,11,4,12,14,1,11,2,'Primary Key'),(43,12,4,12,14,1,12,3,'Foreign Key'),(44,12,4,12,14,1,12,4,'Foreign Keyword'),(45,12,4,12,15,3,12,11,'Forward Key'),(46,12,4,12,15,3,12,12,'Fashion Key'),(47,15,4,12,14,1,15,5,'Parent'),(48,15,4,12,15,3,15,6,'Parent');
/*!40000 ALTER TABLE `rpack4` ENABLE KEYS */;
UNLOCK TABLES;
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;

/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;

-- Dump completed on 2020-12-01 23:12:11
[Ljava.lang.String;@3ee2529c
-- Dump execute Failed
creating dump
----SQL backup file generated: mydb_abackup.sql----
2020-12-01 23:38:33.101  INFO 315311 --- [extShutdownHook] j.LocalContainerEntityManagerFactoryBean : Closing JPA EntityManagerFactory for persistence unit 'default'
2020-12-01 23:38:33.108  INFO 315311 --- [extShutdownHook] o.s.s.concurrent.ThreadPoolTaskExecutor  : Shutting down ExecutorService 'applicationTaskExecutor'
2020-12-01 23:38:33.109  INFO 315311 --- [extShutdownHook] com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Shutdown initiated...
2020-12-01 23:38:33.132  INFO 315311 --- [extShutdownHook] com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Shutdown completed.
