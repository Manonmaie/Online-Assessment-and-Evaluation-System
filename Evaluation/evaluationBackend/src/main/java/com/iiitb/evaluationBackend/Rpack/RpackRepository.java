...
public interface RpackRepository extends CrudRepository<Rpack, Integer>{
	...
	@Query(value="SELECT option_percentage FROM apack2 WHERE qp_item_id = :qp_item_id and item_option_id = :item_option_id and option_text =:option_text", nativeQuery=true)
	public Integer fetchItemTFPercentData(@Param("qp_item_id") int qp_item_id, @Param("item_option_id") int item_option_id, @Param("option_text")String option_text);
